...
func _process(delta):
	...
	var packet_size = SteamInit.Steam.getAvailableP2PPacketSize(0)
	while packet_size > 0:
		# There is a packet 
		_read_p2p_packet(packet_size)
		# Check for more available packets
		packet_size = SteamInit.Steam.getAvailableP2PPacketSize(0)
...
func _send_p2p_packet(steam_id, data: PoolByteArray, send_type: int = SteamInit.Steam.P2P_SEND_RELIABLE, channel: int = 0) -> bool:
	return SteamInit.Steam.sendP2PPacket(steam_id, data, send_type, channel)
...
func _read_p2p_packet(packet_size:int):
	...
	var packet = SteamInit.Steam.readP2PPacket(packet_size, 0)
